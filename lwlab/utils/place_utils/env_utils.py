...
def set_robot_to_position(env, global_pos, global_ori, keep_z=True, env_ids=None):
    ...
    env.cfg.robot_cfg.init_state.pos = robot_pos
    env.cfg.robot_cfg.init_state.rot = robot_quat
...
def set_camera_follow_pose(env, offset, lookat):
    if env.cfg.robot_base_link is not None:
        ...
        robot_mat = T.quat2mat(env.scene.articulations["robot"].data.body_com_quat_w[..., 0, :][0].cpu().numpy()[[1, 2, 3, 0]])
